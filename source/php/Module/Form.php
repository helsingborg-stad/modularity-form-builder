...
class Form extends \Modularity\Module
{
    ...
    public function init()
    {
        ...
        add_action('add_meta_boxes', [$this, 'metaBoxResponses'], 10, 2);
        add_action('current_screen', [$this, 'export']);
        add_action('wp_ajax_get_selected_field', [$this, 'getSelectedField']);
    }
    ...
    public function data(): array
    {
        ...
        $data['submissionResult'] = isset($_GET['form']) ? wp_kses($_GET['form'], []) : false;
        ...
        $data['lang'] = [
            'errorMessage' => __(
                'Something went wrong. Please look over the provided information and try again.',
                'modularity-form-builder',
            ),
        ...
        foreach ($data['form_fields'] as $key => &$field) {
            ...
            if ($field['acf_fc_layout'] === 'sender') {
                ...
                $field['gridClass'] = $field['columns'] === true ? 'o-grid-6@md' : 'o-grid';
            }
    ...
    public function metaBoxResponses($postType, $post)
    {
        ...
        add_meta_box(
            'form-responses',
            __('Responses', 'modularity-form-builder'),
            [$this, 'showResponses'],
            $postType,
            'normal',
            'high',
    ...
    private function setAttributeList($field)
    {
        ...
        if (!empty($field['value_type']) && $field['value_type'] === 'date') {
            $field['attributeList']['min'] = \ModularityFormBuilder\Helper\SanitizeData::formatDate(
                $field['min_value'],
            );
            $field['attributeList']['max'] = \ModularityFormBuilder\Helper\SanitizeData::formatDate(
                $field['max_value'],
            );
        ...
        } elseif (!empty($field['value_type']) && in_array($field['value_type'], ['number', 'range'])) {
    ...
    private function normalizeOptions($options)
    {
        if (count($options) != count($options, COUNT_RECURSIVE)) {
            $return = [];
            array_walk_recursive($options, static function ($value) use (&$return) {
                $return[$value] = $value;
            });
            return $return;
        }
    ...
    public function script()
    {
        ...
        add_action('wp_enqueue_scripts', [$this, 'initScriptsQue'], 30);
    }
