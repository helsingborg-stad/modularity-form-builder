	...
	EditForm.prototype.uploadFile = (
		...
		$.ajax({
			...
			beforeSend: (response) => {
				...
				$(".upload-status", target).html(
					'<div class="spinner spinner-dark is-active" style="float:none;"></div>',
				);
			},
		...
		$.ajax({
			...
			success: (response) => {
				if (response.success) {
					$form
						.find('button[type="submit"]')
						.append('<i class="pricon pricon-check"></i>')
						.find(".spinner")
						.hide();
				} else {
					$(".modal-footer", $form).html(
						'<span class="notice warning"><i class="pricon pricon-notice-warning"></i> ' +
							response.data +
							"</span>",
					);
				}
